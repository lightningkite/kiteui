...
class SsrGoldenSnapshotTest {
    ...
    /** Locates (without requiring it to already exist) `example-app/testResources@jvm/golden/<name>.html`. */
    private fun goldenFile(name: String): File {
        val marker = "src/com/lightningkite/mppexampleapp"
        var base: File? = File(".").absoluteFile
        repeat(6) {
            base?.let { b ->
                for (moduleRoot in listOf(b, File(b, "example-app"))) {
                    if (File(moduleRoot, marker).isDirectory) {
                        return File(moduleRoot, "testResources@jvm/golden/$name.html")
                    }
                }
            }

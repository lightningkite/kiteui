...
public fun String.applyLocalizations(local: Set<NeededStringTemplate>): String {
    var value = this
    for (l in local) {
...
public fun String.localizer(out: MutableSet<NeededStringTemplate>) {
    val raw = this
...
public data class NeededStringTemplate(
    ...
    public fun useStart() = if (args.isEmpty()) "Strings.$name" else "Strings.$name("
    public fun useEnd() = if (args.isEmpty()) "" else ")"
    public fun use(inputs: List<String>) = if (args.isEmpty()) "Strings.$name" else "Strings.$name(${inputs.joinToString()})"
...
public class CodeData(var braceLevel: Int = 1, var parenLevel: Int = 1) {
}

public class StringLitData(
    public val start: Int,
) {
    public var triple = false
    public val builder = StringBuilder()
    public val content = ArrayList<String>()
    public fun append(c: Char) = builder.append(c)
    public fun finishSection() {
        content += builder.toString()
        builder.clear()
    }

    public val args = ArrayList<String>()
}


val `casing separator regex` = Regex("([-_\\s]+([A-Z]*[a-z0-9]+))|([-_\\s]*[A-Z]+)")
public inline fun String.caseAlter(crossinline update: (after: String) -> String): String =
...
public fun String.titleCase() = caseAlter { " " + it.capitalize() }.capitalize()
public fun String.spaceCase() = caseAlter { " " + it }.decapitalize()
public fun String.kabobCase() = caseAlter { "-$it" }.toLowerCase()
public fun String.snakeCase() = caseAlter { "_$it" }.toLowerCase()
public fun String.screamingSnakeCase() = caseAlter { "_$it" }.toUpperCase()
public fun String.camelCase() = caseAlter { it.capitalize() }.decapitalize()
public fun String.pascalCase() = caseAlter { it.capitalize() }.capitalize()
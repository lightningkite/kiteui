    ...
    walkTopDown().forEach { file ->
        ...
        when (relativeFile.extension) {
            ...
            "otf", "ttf" -> {
                ...
                val siblingsCommonPrefix =
                    siblings.map { it.nameWithoutExtension }.reduceOrNull { a, b -> a.commonPrefixWith(b) } ?: ""
                ...
                val weight = boldnessNamesByLength.find { (key, value) ->
                    file.nameWithoutExtension.lowercase().contains(key)
                }?.value ?: 400
...
internal data class NeededStringTemplate(
    ...
    }${content.last()}".let { if (it.firstOrNull()?.isLowerCase() == true) "lowercase $it" else it }.split(' ')

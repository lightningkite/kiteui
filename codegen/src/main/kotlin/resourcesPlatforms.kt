...
internal fun resourcesCommon(resourceFolder: File, out: File, packageName: String) {
    var usesBlob = false
...
internal fun resourcesJs(gitIgnores: List<File>, resourceFolder: File, out: File, packageName: String) {
    var usesBlob = false
...
internal fun resourcesAndroid(
    resourceFolder: File,
    androidResFolder: File,
    outKt: File,
    packageName: String,
    // Emit runtime name lookups instead of `R` constants, for builds where no generated `R`
    // exists. Off by default so Gradle builds keep compile-time checking and identical output.
    ...
    val lines = resources
        .joinToString("\n    ") {
            when (val r = it.value) {
                is Resource.Font -> if (lookUpResourcesByName) "actual val ${r.name}: Font = Font { AndroidAppContext.applicationCtx.resources.getFont(resId(\"${it.key.snakeCase()}\", \"font\")) }" else "actual val ${r.name}: Font = Font { AndroidAppContext.applicationCtx.resources.getFont(R.font.${it.key.snakeCase()}) }"
                is Resource.Image -> if (lookUpResourcesByName) "actual val ${r.name}: ImageResource get() = ImageResource(resId(\"${it.key.snakeCase()}\", \"drawable\"))" else "actual val ${r.name}: ImageResource = ImageResource(R.drawable.${it.key.snakeCase()})"
                is Resource.Video -> if (lookUpResourcesByName) "actual val ${r.name}: VideoResource get() = VideoResource(resId(\"${it.key.snakeCase()}\", \"raw\"))" else "actual val ${r.name}: VideoResource = VideoResource(R.raw.${it.key.snakeCase()})"
                is Resource.Audio -> if (lookUpResourcesByName) "actual val ${r.name}: AudioResource get() = AudioResource(resId(\"${it.key.snakeCase()}\", \"raw\"))" else "actual val ${r.name}: AudioResource = AudioResource(R.raw.${it.key.snakeCase()})"
                is Resource.ImageVector -> "actual val ${r.name}: ImageVector = ${r.imageVectorActual}"
                is Resource.Binary -> {
                    usesBlob = true
                    ...
                    val rawRef = if (lookUpResourcesByName) "resId(\"${it.key.snakeCase()}\", \"raw\")" else "R.raw.${it.key.snakeCase()}"
                    "actual suspend fun ${r.name}(): Blob = Blob(AndroidAppContext.applicationCtx.resources.openRawResource($rawRef).readBytes(), \"$mimeType\")"
                }
            }
        }
    ...
    val resIdHelper = if (lookUpResourcesByName) "\n" + """
    // Looked up by name because no generated `R` exists here: an Android library built by the
    // Kotlin Toolchain never invokes AGP. Throws rather than returning 0, so a missing resource
    // fails at the call site instead of as an opaque Android error much later.
    private fun resId(name: String, type: String): Int =
        AndroidAppContext.applicationCtx.let { it.resources.getIdentifier(name, type, it.packageName) }
            .also { if (it == 0) throw IllegalStateException("Android resource " + type + "/" + name + " is missing; resource shrinking strips name-only lookups.") }

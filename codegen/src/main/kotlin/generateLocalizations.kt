...
internal fun generateLocalizations(toRead: List<File>, outKt: File, packageName: String) {
...
//    val commaWithoutSpace = Regex(",[^ ]")
    ...
    toRead.filterNotNull().asSequence()
        ...
        .filter { it.extension == "kt" && it.isFile }
        .forEach {
            it.writeText(it.readText().applyLocalizations(packageName, localizations))
        }
}

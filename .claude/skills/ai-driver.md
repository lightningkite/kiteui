...
kiteui-drive snapshot <app> --interactiveOnly         # Only actionable elements
kiteui-drive snapshot <app> --format Json             # Machine-readable JSON
```

Snapshot output uses compact format:
```
Page: HomePage ()
...
- `= "value"` only shown when accessibilityValue is set
- `[actions]` inline after type (e.g. `[click,longClick]`)
- `(disabled)` / `(hidden)` as flags when applicable

**Component IDs** use relative paths:
...
- **`--interactiveOnly`** prunes structural containers (leaves with no name/value/actions, collapses single-child wrappers)

### Finding Components
```bash
kiteui-drive find <app> --value "Email Code"           # Find by displayed value
kiteui-drive find <app> --type Button                  # Find by component type
kiteui-drive find <app> --action setValue               # Find by available action
kiteui-drive find <app> --id "logout"                  # Find by ID substring
kiteui-drive find <app> --value "OK" --type Button     # Combine filters
kiteui-drive find <app> --type Button --limit 5        # Limit results
kiteui-drive find <app> --type TextInput --format Json  # JSON output
```

**Output format (Text)**:
```
login-btn Button [click,longClick]
logout-btn Button = "Log Out" [click,longClick]
```

**On no match**: returns exit code 1 and prints `NOT_FOUND:` followed by the full snapshot, so scripts can see what's actually on screen.

**Use `find` instead of grep/sed on snapshot text** — it's stable across format changes and returns machine-parseable results.
...
kiteui-drive perform <app> scroll <targetId> --dx 0 --dy -200
kiteui-drive perform <app> dragAndDrop <targetId> --toTargetId <destId>  # drag and drop to element
...
| Method | Returns | Description |
|--------|---------|-------------|
| `findById(id)` | `UiComponent?` | Search by exact ID, suffix, or anchored prefix |
...
data class UiComponent(
    val id: String,            // relative path ID (e.g. "navigatorView/0/3" or "submitBtn")
    val type: String,          // "Button", "Text", "TextInput", "Column", "Row", etc.
    val value: String?,        // accessibilityValue only (name is in the id, not the value)
    val enabled: Boolean,      // interactive?
    val visible: Boolean,      // on screen?
    val actions: Set<String>,  // {"click", "setValue", "longClick", "scroll", "dragAndDrop"}
...
| `kiteui-drive find <app> --value "X" --type Button` | `snapshot().findAll { it.value?.contains("X") == true && it.type == "Button" }` |

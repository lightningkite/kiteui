...
class Recording {
    ...
    fun start() { isActive = true; commands.clear() }
...
private fun CliCommand.toUiTestScope(): String? = when (this) {
    ...
    is CliCommand.Wait -> {
        when {
            page != null -> "waitForPage(\"$page\")"
            component != null -> "waitForComponent(\"$component\")"
            componentGone != null -> "waitForComponentGone(\"$componentGone\")"
            enabled != null -> "waitForEnabled(\"$enabled\")"
...
private fun UiAction.toUiTestScope(): String = when (this) {
    is UiAction.Click -> "click(\"$targetId\")"
    is UiAction.LongClick -> "perform(UiAction.LongClick(\"$targetId\"))"
    is UiAction.SetValue -> "setValue(\"$targetId\", \"${value.replace("\"", "\\\"")}\")"
    is UiAction.Scroll -> "perform(UiAction.Scroll(\"$targetId\", ${dx}f, ${dy}f))"
    is UiAction.Navigate -> "navigate(\"$route\")"

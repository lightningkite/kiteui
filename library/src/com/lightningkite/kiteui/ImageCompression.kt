...
internal fun calculateScaledSize(
    ...
    // by Claude - guard against zero/negative dimensions
    require(originalWidth > 0 && originalHeight > 0) { "Original dimensions must be positive, got ${originalWidth}x${originalHeight}" }
    require(maxWidth > 0 && maxHeight > 0) { "Max dimensions must be positive, got ${maxWidth}x${maxHeight}" }
    ...
    return maxOf(1, (originalWidth * scale).toInt()) to maxOf(1, (originalHeight * scale).toInt())
}

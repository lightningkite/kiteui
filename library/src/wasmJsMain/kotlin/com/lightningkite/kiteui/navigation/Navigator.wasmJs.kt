...
actual fun ScreenStack.bindToPlatform(context: NContext) {
    ...
    window.addEventListener("popstate", { event ->
        ...
        try {
            suppressNav = true
            (event.state as JsNumber?)?.toInt()?.let {
                ...
                if (added.isNotEmpty()) {
                    for ((index, new) in added.withIndex()) {
                        routes.render(new)?.urlLikePath?.render()?.let {
                            ...
                            window.history.pushState((index + indexWhereChangesStart).toJsNumber(), "", basePath + it)
                        }
                    }
                }
            }
        ...
        reactiveScope {
            ...
            s.lastOrNull()?.let { routes.render(it) }?.let {
                ...
                window.history.replaceState(
                    s.lastIndex.toJsNumber(), "", basePath + it.urlLikePath.render()
                )
            }
        }
    }
...
@Serializable data class BaseUrlScript(
    val baseUrl: String
)

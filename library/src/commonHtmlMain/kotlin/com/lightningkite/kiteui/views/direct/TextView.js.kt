...
actual abstract class TextView actual constructor(context: RContext) : RView(context) {
    ...
    actual var ellipsis: Boolean = true
        set(value) {
            ...
            if(value)
                native.style.record.set("overflow", "hidden")
            else
                native.style.record.set("overflow", null)
        }
    actual var wraps: Boolean = true
        set(value) {
            field = value
            native.style.record.set("text-wrap", if(value) "wrap" else "nowrap")
            native.style.record.set("text-wrap-mode", if(value) "wrap" else "nowrap")
        }
}

...
actual class NumberField actual constructor(context: RContext) : RView(context) {
    ...
    actual val content: Writable<Double?> = object : Writable<Double?>, BaseListenable() {
        init {
            native.events["input"] = {
                numberAutocommaRepair(
                    dirty = native.attributes["value"] ?: "",
                    selectionStart = selectionStart,
                    selectionEnd = selectionEnd,
                    setResult = {
                        native.attributes["value"] = it
                    },
        ...
        override val state: ReadableState<Double?> get() = ReadableState(native.attributes["value"]?.filter { it.isDigit() || it == '.' }?.toDoubleOrNull())
        override suspend fun set(value: Double?) {
            native.attributes["value"] = value?.commaString()
        }
    }
    ...
    actual var range: ClosedRange<Double>? = null
        set(value) {
            field = value
            value?.let {
                native.attributes["min"] = it.start.toString()
                native.attributes["max"] = it.endInclusive.toString()
            } ?: run {

 ...
 */
// by Claude - use "command" discriminator to avoid collision with Find.type field
@OptIn(ExperimentalSerializationApi::class)
@JsonClassDiscriminator("command")
@Serializable
sealed class CliCommand {
    /** List all currently connected app instances. */
    // by Claude - renamed from List to ListApps to avoid shadowing kotlin.collections.List
    @Serializable @SerialName("list")
    data class ListApps(val format: ListFormat = ListFormat.Text) : CliCommand() {
        @Serializable
    ...
    data class Snapshot(
        ...
        // by Claude - --interactiveOnly flag prunes structural-only containers from output
    ...
    /**
     * Find components matching criteria in the UI tree.
     * Returns matching component IDs (one per line in Text, array in Json).
     * CLI: `kiteui-drive find <appId> [--value text] [--type Button] [--action click] [--limit 10]`
     */
    // by Claude - structured component search, avoids fragile grep/sed on snapshot text
    @Serializable @SerialName("find")
    data class Find(
        val appId: String,
...
// by Claude - result of a find command: component ID + context for identification
@Serializable

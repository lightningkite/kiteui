...
public actual class Select actual constructor(context: ElementContext): NativeInteractiveElement(context) {
    ...
    override fun nativeApplyTheme(theme: ThemeAndBack) {
        ...
        dropdownArrow(native.resources.displayMetrics.density).apply {
...
//            }
//
//            val dropdown = dropdownArrow(native.resources.displayMetrics.density)
...
// Drawn rather than loaded from res/: an Android library module compiled by the Kotlin Toolchain
// never invokes AGP, so no R class -- its own or a dependency AAR's -- is on the compile classpath.
// Geometry matches the 24dp baseline_arrow_drop_down_24 vector it replaces; the caller's
// PorterDuff colour filter still applies, as it did to the vector's tint.
private fun dropdownArrow(density: Float): ShapeDrawable {
    val path = Path().apply {
        moveTo(7f, 10f)
        lineTo(12f, 15f)
        lineTo(17f, 10f)
    ...
    return ShapeDrawable(PathShape(path, 24f, 24f)).apply {
        val side = (24 * density).toInt()
        setIntrinsicWidth(side)
        setIntrinsicHeight(side)
    }
}

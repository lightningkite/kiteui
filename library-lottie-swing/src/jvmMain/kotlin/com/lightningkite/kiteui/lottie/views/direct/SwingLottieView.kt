...
class SwingLottieView(
    ...
    override val native: JPanel = panel

    private val _state = RawReactive<Unit>()
    val state: Reactive<Unit> = _state

    private val _duration = RawReactive<Duration?>(ReactiveState(null))
    val duration: Reactive<Duration?> = _duration

    private val _playing = Signal(false)
    val playing: MutableReactive<Boolean> = object : MutableReactive<Boolean> {
    ...
    val progress: MutableReactive<Float> = object : MutableReactive<Float> {
    ...
    val completedPlay: Listenable = object : Listenable {
...
// DSL functions for Swing
fun ViewWriter.swingLottie(
    source: LottieSource,
    description: String,
    setup: SwingLottieView.() -> Unit = {}
): SwingLottieView = write(SwingLottieView(context, source, description), setup)

fun ViewWriter.swingLottie(
    url: String,
    description: String,
    setup: SwingLottieView.() -> Unit = {}
): SwingLottieView = swingLottie(LottieRemote(url), description, setup)
